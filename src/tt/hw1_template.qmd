...
Interpretation : Part-d (b) --- In this facet's scatter plot, we're comparing humidity of 9 am for different cities.                                                #We can see that Adeliade show's lower humidity at 9 am when temperatures of 9am and temperatur of 3pm are hotter. So if morning in Adelaide starts warmer and stays warmer till after, then humidity 9 am,  with certain probablity will be lower. Canberra, Hobart, Melbourne, Canberra and Syndey shows somewehat stable humidity at 9 am with very minor fluctuations when morning temperature and afternoon temperature increases.
...
```
```{r}
# creating the residual plot for 
weather_model_2%>% 
  augment(new_data = weather_data) %>% 
  ggplot(aes(x = .pred, y = .resid)) +
  geom_point() + 
